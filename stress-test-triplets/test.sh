...
WF=(   "wf1"  "wf1"  "wf1"  "wf1"  "wf1"  "wf1"  "wf2"  "wf2"  "wf2" )
RPS=(  "300"  "150"  "375"  "100"  "125"  "150"  "175"  "200"  "225" )

WF2=(  "wf2"  "wf2"  "wf2"  "wf3"  "wf3"  "wf3"  "wf3"  "wf3"  "wf3" )
RPS2=( "100"  "350"  "325"  "100"  "400"  "350"  "150"  "275"  "300" )
...
for j in ${!RPS[@]}; do
  ...
  do
    ...
    python3.8 ./helpers/polling-script.py $start_offset 15 $end_offset ./$workdir/${WF[j]}-${RPS[j]}-${WF2[j]}-${RPS2[j]}-${WF3[j]}-${RPS3[j]}-$now-iteration-$i-pods.csv ./$workdir/${WF[j]}-${RPS[j]}-${WF2[j]}-${RPS2[j]}-${WF3[j]}-${RPS3[j]}-$now-iteration-$i-nodes.csv ./$workdir/${WF[j]}-${RPS[j]}-${WF2[j]}-${RPS2[j]}-${WF3[j]}-${RPS3[j]}-$now-iteration-$i-pods-no-istio.csv >> ./$workdir/out-$now.txt $shifts &
    ...
    ./delete-3B-1E-param.sh $w1	$w2 $w3	$w4

    sleep 3
    printf "Deleted currencyservice and frontend deployments. Now recreating...\n"

    ./create-3B-1E-param-nodes.sh $w1 $w2 $w3 $w4
    ...
    sleep $interval
    printf "Finished workflows ${WF[j]} at rate ${RPS[j]} with ${WF2[j]} at rate ${RPS2[j]} with ${WF3[j]} at rate ${RPS3[j]} - iteration $(($i-1))\n"
  done
...
if [ "$turnover" == true  ]
then
  ./delete-3B-1C-param.sh $w1 $w2 $w3 $w4
  sleep 5
  tmp=$w1
  w1=$w2
  w2=$w3
  ...
  shifts=$(($shifts+1))
  printf "\nShifting nodes...\n"
  ./create-3B-1C-param-nodes.sh $w1 $w2 $w3 $w4
  sleep 30
else
  break
fi
